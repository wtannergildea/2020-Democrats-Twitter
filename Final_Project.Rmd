...
write_rds(fav_average, "2020-Dems/fav_average")
...
summary_table <- tweets %>% 
  ...
  mutate(tweet_length = str_length(text)) %>%
  mutate(mean_tweet_length = mean(tweet_length)) %>% 
  mutate(fav_average = mean(favoriteCount)) %>% 
  mutate(rt_average = mean(retweetCount))  
...
clean_summary_table <- summary_table %>% 
  ...
  # summarize("Total" = tweet_count,
  #           "Average Length" = mean(tweet_length),
  #           "Average Favorites" = fav_average,
  #           "Average Retweets" = rt_average)
